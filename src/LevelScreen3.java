...
public class LevelScreen3 extends BaseScreen {
	...
	private ArrayList<SpawnMur> spawnsMur;
	boolean sonido;
	Label coinLabel;
	Label vidasLabel;
	Label muertesLabel;
 ...
 public LevelScreen3(int vidas,int coins,boolean powerup1,boolean powerup2,boolean sonido) {
	 this.coins = coins;
	 this.vidas = vidas;
	...
	public void initialize() {
		tma = new TilemapActor("assets/Nivel3.tmx", mainStage);
		...
		//		coinImage.setBounds(50,70,400,330);
		...
		try {
//			AudioInputStream audioInputStream = AudioSystem.getAudioInputStream(new File("assets/Sonidos/rally-x.wav"));
...
//			clip.loop(1);
		...
		spawnsMur = new ArrayList<SpawnMur>();
		Enemigo enemigo;
		SpawnGob spawngob;
		SpawnSeta spawnseta;
		SpawnMur spawnmur;
		...
		for (MapObject obj : tma.getRectangleList("spawnMur")) {
			MapProperties props = obj.getProperties();
			spawnmur= new SpawnMur((float) props.get("x"), (float) props.get("y"), mainStage, this);
			spawnsMur.add(spawnmur);
		}
	...
	public void update(float dt) {
		...
		for (BaseActor door : BaseActor.getList(mainStage, "Puerta")) {
			if (jugador.overlaps(door) ) {
				
				BaseGame.setActiveScreen(new Final());
...
for (SpawnMur spawnMur : spawnsMur) {
